...
def extractincidents(data):
    ...
    if not os.path.exists('src/backend/resources/data.tsv'):
        with open('src/backend/resources/data.tsv', 'w') as file:
            file.write(temp_text)

    # Write the extracted data to a file
    with open('src/backend/resources/data.tsv', 'a') as file:
        file.write(temp_text)
...
def pca_conversion3(data):
    pca = PCA(n_components=3)  # Reduce to 2 components for visualization
    principal_components = pca.fit_transform(data.drop(columns=['Cluster']))
    pca_data = pd.DataFrame(data=principal_components, columns=['PC1', 'PC2'])
    pca_data['Cluster'] = data['Cluster']
    return pca_data
